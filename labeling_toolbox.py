...
class ImageHomography:
    def __init__(self, image_points_file):
        self.image_points_file = image_points_file
        if str(self.image_points_file).lower().endswith('csv') is False:
            image_points_df = None
            print("Input file for homography must be CSV file type")
        else:
            image_points_df = pd.read_csv(Path(self.image_points_file))

        image_paths = list(image_points_df.index.values)
        # loop to align each image. Needs alignment image
