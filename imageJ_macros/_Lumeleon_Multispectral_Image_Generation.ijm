...
//  USER OPTIONS
	...
	Dialog.create("Multispectral Image Generation from machine labeled standards");
...
// get directory of machine labeled points corresponding to RAW images

cellSeparator=",";
lineSeparator="\n";

machineStandardsPath=File.openDialog("Select machine labeled standards csv");
Table.open(machineStandardsPath);


// get table headings as tab delimited string
columnHeadingsString = Table.headings();
columnHeadings=split(columnHeadingsString, "\t");

image_path=Table.getColumn(columnHeadings[0]); // return image path column as Array
...
// arrays to hold overexposure warnings for each channel, returned as table at end
overexposure_red=newArray(image_path.length);
overexposure_blue=newArray(image_path.length);
overexposure_green=newArray(image_path.length);

// construct path to save results warning table to
savingPathTemp = split(machineStandardsPath, "/");
	if(savingPathTemp.length == 1) // windows
		savingPathTemp = split(machineStandardsPath, "\\");
saveFolder=replace(machineStandardsPath, savingPathTemp[savingPathTemp.length-1], "");
...
for(overall_row=0; overall_row<image_path.length; overall_row++){
	// overall_row=i;
	...
	for(j=0; j<nPhotos; j++){
		...
		if(imagesSequential == 1){
			...
			imagePath=image_path[overall_row];
			imagePathTemp = split(imagePath, "/");
			if(imagePathTemp.length == 1) // windows
				imagePathTemp = split(imagePath, "\\");
	...
	for(i=0; i<standardLevels.length; i++){
		// need to store value that overall image path loop is on to get correct bounding box
		...
		if( customiseLocation == 1 || j == 0 || greyLocation == "Separate photos"){ // first image, or custom location
			...
			standardArray=Table.getColumn(columnHeadings[i]);
			coordinates=standardArray[overall_row];
		...
		if(j > 0 && autoAlignOption != "None" && greyLocation == "Same photo" && customiseLocation == 0 && autoAlignOption != "Affine Align"){ // control for alignment of grey standard selection
			...
			// maybe move these down where coords are established
			xCoords=coordinate[0:2];
			yCoords=coordinates[2:4];
		...
		for(k=1; k<=3; k++){
			if(parseInt(photoSettings[k]) > 0){ // channel is to be added
				...
				if(nonLin == "RAW Photo" && max >= 65535){
					// warning text returned as table of warnings to not interupt batch processing
					warningText = "This standard could be over-exposed in this channel";
				} else {
					warningText = "";
				}

				if(k == 1){ // red
					overexposure_red[overall_row]=warningText;
					rPxs[i] = mean;
				}
				if(k == 2){ // green
					overexposure_green[overall_row]=warningText;
					gPxs[i] = mean;
				}
				if(k == 3){ // blue
					overexposure_blue[overall_row]=warningText;
					bPxs[i] = mean;
				}
			}
		}//k
	...
	// Overwrite if file exist to avoid user interaction
	// Bandaid for blank image name. Fix when supporting UV images
	imageName = split(image_path[overall_row], ".");
	configFilePath =  imageName[0] + ".mspec";
...
// return results table with image paths and overexposure warnings for each table
Table.create("BatchResults");
csvPath=saveFolder + "mica_results.csv";
Table.setColumn("Image paths", image_path);
Table.setColumn("Warn R", overexposure_red);
Table.setColumn("Warn B", overexposure_blue);
Table.setColumn("Warn G", overexposure_green);
Table.save(csvPath);
showStatus("Finished processing images");
